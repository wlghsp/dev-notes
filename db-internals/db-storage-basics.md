...
트래픽이 몰리면 DB가 느려진다. CPU는 여유 있고, 메모리도 남는다. 근데 왜 느린가?

대부분 디스크 I/O다. DB는 데이터를 디스크에서 읽어오는데, 디스크는 메모리보다 수천~수십만 배 느리다. Buffer Pool이 작으면 매 쿼리마다 디스크를 친다. Page 구조를 모르면 왜 16KB를 통째로 읽는지, 왜 Random I/O가 Sequential보다 느린지 설명이 안 된다.

이후 나오는 B-Tree, WAL, 트랜잭션은 전부 이 디스크 I/O 비용을 줄이기 위한 구조다.
