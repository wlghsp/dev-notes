...
**Buffer**는 쓰기 대기가 목적이다. CPU처럼 빠른 쪽이 데이터를 생산하면 바로 디스크에 쓰지 않고 Buffer에 모아뒀다가 나중에 한꺼번에 디스크로 내려보낸다.

**Cache**는 읽기 재사용이 목적이다. 디스크처럼 느린 쪽에서 한 번 읽어온 데이터를 메모리에 들고 있다가 같은 데이터 요청이 오면 디스크를 다시 거치지 않는다.
