...
쿼리 하나가 갑자기 느려졌다. `EXPLAIN`을 봤더니 인덱스를 타고 있고, rows 수도 적다. 근데 왜 느린가?

Lock 경쟁 때문일 수도 있고, Buffer Pool이 작아서 매번 디스크를 치는 것일 수도 있고, Optimizer가 잘못된 실행 계획을 선택한 것일 수도 있다. DBMS 컴포넌트 구조를 모르면 이 세 가지를 구분할 수 없다. 숫자는 읽히는데 원인은 안 보이는 상태가 된다.

어느 레이어에서 병목이 생기는지 알아야 어디를 봐야 하는지 알 수 있다.
